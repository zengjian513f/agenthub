...
3. 节点注册和移除只在服务器上操作，通过中央 checkout 里的 `Registry`，凭据不经过网页。
   公开机器列表不返回节点连接地址。网页能改的是机器的名称、配色、终端后端和是否启用，
   见「设置 → 机器」；地址和凭据不在网页的可改范围内。注册信息仍保存为权限 `0600` 的
   `~/.local/share/agenthub/hub-nodes.json`，节点凭据不会发送到浏览器。
   每台机器前有一个启用勾选框：取消勾选的机器对中央和页面视同不存在——不在机器筛选、
   会话列表、搜索、新建会话和终端能力里出现，监控不再探它，按 id 直连返回 404，健康状态清空；
   它只留在设置页里等着重新勾选（勾上即刻重新检查）。双系统的机器（同一台硬件装两个系统，
   开着一个另一个必然关着）就各注册一条，只勾当前开着的那个。`enabled` 随注册表落盘，重启后不变。
   机器的顺序只由用户决定：拖动每行左边的 ⋮⋮ 把手（键盘上按 ↑ ↓）排序，机器筛选、新建会话下拉
   和设置页都按这个顺序；启用或停用不改变顺序，停用的机器留在原位。顺序通过
   `POST /api/nodes/order`（`{"ids": [...]}`，必须是全部机器的一个排列）写回注册表。
...
- 机器关机是常态。Hub 自带节点监控线程，每 10 秒探测所有节点的 `/api/sessions`
  （带上次 sig 的条件请求，列表没变时节点只回几十字节的 `unchanged`；页面轮询同样命中 Hub 缓存），
