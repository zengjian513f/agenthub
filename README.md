...
- **右栏**：消息流。用户/助手正常展开，思考、系统提示、注入上下文（AGENTS.md、`<user_info>`、`<system-reminder>` 之类）默认折叠，点标题栏展开。超长消息先截断，可展开全文；会话一次完整载入，并由浏览器端 LRU 控制内存占用。
...
- **工具调用分组**：连续 3 条以上的工具调用/输出自动并成一组，组头显示「N 次工具调用」和工具名摘要（如 `Write ×3 · Bash ×2 · Edit ×3`），整组默认折叠。展开后组内每条仍可单独展开。这样一屏能看到完整的对话脉络，而不是被几十条工具输出淹没。
- **管理操作**：复制原始文件路径、导出 Markdown、全部折叠/展开、合并子代理消息、删除会话。

## 界面状态

左右栏之间的分割线可拖动（下限 200px，右侧至少留 320px），**双击复位**到默认 340px。
...
启用后，任何会话的详情页都会出现「**⌨ 接管**」按钮。点一下，服务端在后台把这个会话用 tmux 起起来（`claude --resume` / `codex resume` / `grok --resume`，自动沿用它原来的工作目录），**消息流底部立刻出现一个输入框** —— 打字回车就发给会话，回复通过增量同步自动出现在上面的历史里。

按钮状态跟着变：没接管时是「⌨ 接管」，接管过就是「⌨ 打开输入」。

输入框旁边还有两个键：`Esc` 发中断，`⌨` 展开**完整终端**。日常对话用输入框就够；需要方向键选菜单、回答批准提示、看 TUI 全屏界面时，展开终端。
...
底部终端可拖动调整高度（记在 localStorage）。「收起」只是断开，tmux 会话继续跑；要真正结束用「结束会话」。切换到别的会话时终端自动收起。
...
`tests/e2e.py` 用 Playwright 驱动真实 Chromium 点遍全部交互（209 项断言：列表渲染、来源筛选、两种视图、分组折叠、标题过滤、全文搜索、消息折叠规则、全部折叠/展开、展开全文、公式与图片、复制、导出、子代理合并、增量同步、刷新、快捷键、接管终端、删除入回收站，以及页面无 JS/HTTP 错误）。
