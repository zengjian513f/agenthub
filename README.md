...
- **三级宽度排版，折叠不留白**：顶栏和会话标题栏在任何宽度下都只占一行，按视口宽度分三级决定筛选和固定平铺的操作，但任何东西都只在真的放不下时才折进 ⋯，不会一边折叠一边留白。宽屏（≥1200px）机器、来源、视图筛选全部平铺，会话操作全部平铺在标题栏；中屏（721–1199px）机器筛选收成一个下拉按钮（按钮上概括当前选中的机器，列表仍支持多选与双击单选），来源和视图只留图标，标题栏固定平铺星标；窄屏（≤720px）标题栏固定只留终端。顶栏右侧的新建、重新扫描、回收站、报告问题、设置按钮按实际宽度排：筛选条放得下就全部平铺、⋯ 不出现；放不下才从设置起往前折，折到筛选条不再被挤压为止（机器/来源筛选增减、会话计数变宽、新建按钮出现都会重新量）。会话元信息按 消息数、大小、起止时间、机器、目录、来源、模型、会话号、分支 的固定顺序直接跟在标题后面，宽屏/中屏标题至少保留 min(8em, 40%) 宽度，窄屏标题不让位、元信息只填标题右边的空白，放不下的从末尾起收进标题栏 ⋯；元信息全放下了还有空位，菜单里剩下的操作也按顺序平铺出来，菜单空了 ⋯ 不显示；拖动分割线、开合左栏或改窗口宽度时实时进出。⋯ 里的元信息可以划选复制。运行状态是标题图标右上角的绿/蓝点，和左栏列表一致。
