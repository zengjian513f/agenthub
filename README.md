...
- **三级宽度排版**：顶栏和会话标题栏在任何宽度下都只占一行，按视口宽度分三级。宽屏（≥1200px）机器、来源、视图筛选全部平铺，右侧五个按钮全露出，会话操作全部平铺在标题栏；中屏（721–1199px）机器筛选收成一个下拉按钮（按钮上概括当前选中的机器，列表仍支持多选与双击单选），来源和视图只留图标，回收站、报告问题和设置折进顶栏 ⋯，标题栏只平铺星标；窄屏（≤720px）顶栏五个按钮全部折进 ⋯，标题栏只留终端和 ⋯。会话元信息按 消息数、大小、起止时间、机器、目录、来源、模型、会话号、分支 的固定顺序直接跟在标题后面，标题至少保留 min(8em, 40%) 宽度，剩余宽度放不下的从末尾起收进标题栏 ⋯（宽屏元信息全放下时 ⋯ 不显示）；拖动分割线、开合左栏或改窗口宽度时实时进出。⋯ 里的元信息可以划选复制。运行状态是标题图标右上角的绿/蓝点，和左栏列表一致。
- **手机适配**：720px 以下改为“会话列表 → 会话详情”的单栏导航，不再硬挤左右栏，并记住当前在列表还是详情，刷新后原页恢复。详情使用紧凑返回键，操作和元信息收进标题栏 ⋯ 菜单；输入框使用短提示词，手机 Enter 只换行、点击发送按钮才提交，并兼容安全区域；展开终端时直接覆盖消息区和输入框，提供 Ctrl（下一键生效）、Tab、方向、翻页和 Esc 触控键。
