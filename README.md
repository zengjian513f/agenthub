...
Claude Code / Codex / Grok 三家 CLI 会话的统一网页浏览与管理服务。后端只用 Python 标准库（可选装 `orjson`：`pip install orjson`，有预编译 wheel，装了之后会话文件逐行解析快 2.6–2.8 倍，没装则走标准库，行为相同——差异只有超过 64 位的整数会以浮点返回，见 `agenthub/fastjson.py`）；前端为原生 JS，KaTeX 与 xterm.js 静态内置，无在线依赖和构建步骤。
...
文件链接提供独立预览页；Markdown 默认渲染标题、表格和代码块，正文使用无衬线字体，支持切换源码、复制和自动换行。源码与代码块共用网页终端的字体选项，文件管理器预览同样适用。直接通过 API 获取文件仍返回原文；浏览器可用 `raw=1` 查看原文，下载保留原文件。


Codex 回退会生成继承历史的子会话，原会话（父会话）默认从左栏隐藏，不删除记录。
子会话标题栏的 ⑂ 图标下拉列出整条父会话链，每一级右侧的「显示」把它放回左栏，
「隐藏」再收起；显示状态保存在服务端，换浏览器仍然有效。父会话自己的标题栏和右键菜单也能直接隐藏。

## 界面

- **左栏**：两种视图 —— 📁 项目树（按 cwd 分组）/ 🕒 时间轴（按日期倒排，每条单独一行显示所在目录）。分组可折叠，折叠状态存 localStorage。
- **分层（顶栏层级树图标开关）**：打开后每条会话的子代理、以及由它发起的其他 CLI 会话缩进挂在它下面（可多层），跨目录的孩子离开自己的分组；整棵子树按最新活动排位，在跑的孩子排最前。每行行首是引导区：每一级祖先一根竖线，再一个 16px 的三角槽位（叶子留空），根行的三角与分组标题的三角同列，深一层的槽位正好在上一层槽位正下方——靠固定宽度对齐，不用绝对定位。子代理行的来源图标照常，右下角叠一个层级树小标（与顶栏分层开关同一图形 `#i-tree`）；被发起的会话用自己的来源图标。列表元信息不再显示子代理数（`⑂N`），标题栏元信息也不再显示 git 分支（`⑂ 分支名`）；两者仍在 `/api/sessions` 行的 `agents`、`branch` 字段里。发起者行的三角可收起整棵子树（收起状态存 localStorage）。发起者不在当前列表里（被筛掉、已删除）的会话仍作根显示。关掉开关即回到平铺，子代理仍只在详情标题下拉里切换。Claude 因上下文过长 continue 另起的 JSONL（文件尾 `continued-in`）只保留链上最新那一条出现在左栏；被取代的旧文件不展示。这和双 Esc 回退不同：双 Esc 仍在同一份 JSONL 里换枝。同一文件里的 `/compact` 也不另开会话。
- **发起关系怎么来的**：一条会话由另一条会话派出去（Claude 的 Bash 里跑 `codex exec`、`grok -p`、`claude -p`，或经 `term.new_session` / ptyhost 起一条新会话）时，服务端趁两边进程都在，从进程树认出发起者：沿 CLI 主进程的祖先链找另一条会话的主进程，或链上某一级环境里带着别条会话的身份（Claude 给工具子进程设的 `CLAUDE_CODE_SESSION_ID`、`CLAUDE_PID`；Codex 设的 `CODEX_THREAD_ID`、`CODEX_SESSION_ID`——子代理线程的前者是它自己的线程 id，后者才是根会话；Grok 设的 `GROK_SESSION_ID`）。ptyhost 宿主会剥掉 CLI 子进程的会话变量并脱离发起者的进程树，但宿主自己仍带着发起者的环境，所以每一级都看；tmux server 由所有会话共享，走到它就停。一条链上同时看到祖父和父亲时取最晚创建的那条。服务自己的后台线程每 10 秒扫一次（与 `/api/live` 共用 3 秒进程缓存），不依赖有没有网页开着——agent 批量派出的 headless 会话常在没人看页面的几分钟里生灭，只靠浏览器轮询会漏掉。关系一经看到就写进 `session-meta.json`（`spawned_by: {source, sid}`），之后进程退出、重启服务仍保留；`/api/sessions` 行带出该字段，左栏分层用它缩进，详情标题不再放发起者链接。服务启动时会清掉自己环境里的这几个变量，从某条 CLI 会话里手动起服务时网页新建的会话不会被误认成它的孩子。家目录缩写成 `~`，过长的路径中间省略（`/a/b/…/y/z`）——不能用 `direction: rtl` 截左边，bidi 会把开头的 `/` 挪到末尾。
...
- **搜索选项**（搜索框内三个开关，状态记在 localStorage）：`Aa` 大小写敏感、`ab|` 全词匹配、`.*` 正则表达式。前端过滤、后端搜索、正文高亮共用同一套匹配规则。
  ...
  - 搜索在适配器解析后的 Unicode 正文上运行，字面、全词和正则模式语义一致。正文缓存在内存和节点私有磁盘中，服务重启后仍可复用；原生记录、继承历史或确认的回退变化时失效。搜索解析跳过无关工具输出的展示处理。
  - 全部机器搜索立即跳过 Hub 已知离线的节点，在线机器并行搜索；命中逐步显示，进度占独立位置，不遮盖列表。搜索期间仍可新建和打开会话；修改搜索词、取消、刷新列表或进入新建会话会取消旧搜索，迟到的结果不会覆盖新状态。冷扫描期间每秒发送心跳，已显示的匹配在其他节点失败时仍保留。
  - 进度显示已扫描会话数、总数和百分比，以及各机器的准备、扫描、完成、离线跳过或失败状态。总数尚未齐全时只显示真实扫描数量，不显示不定进度动画或估算百分比；达到命中上限也不会把未扫描部分记为完成。
...
- **三级宽度排版，折叠不留白**：顶栏和会话标题栏在任何宽度下都只占一行，按视口宽度分三级决定筛选和固定平铺的操作，但任何东西都只在真的放不下时才折进 ⋯，不会一边折叠一边留白。宽屏（≥1200px）机器、来源、视图筛选全部平铺；中屏（721–1199px）机器筛选收成一个下拉按钮（按钮上概括当前选中的机器，列表仍支持多选与双击单选），来源和视图只留图标；窄屏（≤720px）间距更紧。三档顶栏同高（按钮 30px、内边距 6px），改窗口宽度时不跳高。顶栏右侧的新建、重新扫描、回收站、报告问题、设置按钮按实际宽度排：筛选条放得下就全部平铺、⋯ 不出现；放不下才从设置起往前折，折到筛选条不再被挤压为止（机器/来源筛选增减、会话计数变宽、新建按钮出现都会重新量）。会话标题栏一行的重要程度是 标题 → 操作按钮（星标、折叠过程、报告、搜索、停止/删除）→ 元信息（消息数、大小、起止时间、机器、目录、来源、模型、会话号），各档位相同：宽屏/中屏标题至少保留 min(8em, 40%) 宽度，窄屏标题不让位；标题之后先按顺序平铺按钮，全放下了再把元信息按顺序跟在标题后面，从放不下的那一项起后面的全部收进标题栏 ⋯（元信息先折、按钮后折，某个按钮放不下时元信息一项也不露），菜单空了 ⋯ 不显示；拖动分割线、开合左栏或改窗口宽度时实时进出。⋯ 里的元信息可以划选复制。运行状态是标题图标右上角的绿/蓝点，和左栏列表一致。
- **手机适配**：720px 以下改为“会话列表 → 会话详情”的单栏导航，不再硬挤左右栏，并记住当前在列表还是详情，刷新后原页恢复。详情使用紧凑返回键，操作收进标题栏 ⋯ 菜单、元信息放得下的仍跟在标题后；输入框使用短提示词，手机 Enter 只换行、点击发送按钮才提交，并兼容安全区域；展开终端时直接覆盖消息区和输入框，提供 Ctrl（下一键生效）、Tab、方向、翻页和 Esc 触控键。
- **管理操作**：Claude 子代理可在会话标题处下拉切换，各自保留独立时间线。下拉里主会话固定在前，子代理按结束时间倒序，每行带起止时间；还在跑的子代理没有结束时间、排最前并带绿点，父进程不在则一律不算。运行态由服务端从磁盘判定：Claude 看子代理 transcript 的最后一条记录是否晚于父会话最近一次 task-notification / 前台 Agent tool_result；Codex 子代理与用户线程共用一套回合事件，看它自己 rollout 里最后一条 `task_started` / `task_complete` / `turn_aborted`（`ShutdownComplete` 不落盘，followup 再唤起会写新的 `task_started`）。两家的子代理都跑在父进程内，没有独立进程可查。运行中的会话显示停止按钮，停止后原位变为删除按钮。
...
宽于 720px 的布局（桌面、横屏手机、平板）左右栏之间的分割线可拖动（下限 200px，右侧至少留 320px），**双击复位**到默认 340px。鼠标与手指都能拖：分割线走 pointer 事件并声明 `touch-action: none`，粗指针设备的热区向两侧各扩 6px。
...
| Grok | TUI 维护 `~/.grok/active_sessions.json`。Claude 用 `grok -p` 拉起的 headless 会话不进那份清单、命令行也没有 `--session-id`，靠会话目录里常驻打开的 `events.jsonl`（`chat_history.jsonl` 写完就关）。Grok 主进程不认继承来的 `CLAUDE_CODE_SESSION_ID` |

Claude 写完就关文件，所以**不能只靠 fd**。Grok TUI 同样关 `chat_history.jsonl`，但 headless / `grok -p` 会常驻打开同目录的 `events.jsonl`。Codex 的会话 id 不出现在命令行里，所以也不能只认 session id。扫描约 40ms，服务端缓存 3s。
...
活跃标识按运行环境分色：绿色表示普通的非 tmux 进程，蓝色表示运行在 tmux/宿主中、可直接接入的会话，已退出但还有没看的新内容时未读角标是灰底；颜色只说现在的状态，不随计数固化（未读计数只存条数）。详情只显示纯色点，说明放在悬停提示中。「可接入」看的是这条会话自己的 CLI 是不是某个 pane 的进程：pane 里的 CLI 再派出的 `grok -p` / `codex exec` / `claude -p` 虽然仍在那棵进程树下，但控制台是父 CLI 的，它们算绿点、没有控制台，也不会把父会话的 pane 抢走（`term.process_belongs_to` 与两个后端的 `hosts()` 沿祖先链上行时以别的 CLI 主进程为界）。唯一的例外是 Claude 续写：新 JSONL 的进程跑在原会话 TUI 的守护子进程下，列表只显示续写后的那条，它过继原会话的 pane。
...
- `GET /api/live[?force=1]` — 活跃会话、tmux 子集及当前 CLI 进程启动时间；顺带从进程树记录新出现的发起关系（`spawned_by`，后台线程每 10 秒也会记）
...
诊断包 `attachments/` 留档。随后按报告框选择的 CLI（Claude/Codex/Grok，默认 Codex，
记住上次选择）新建处理会话 tmux；Claude/Codex 用各自的输入框探测确认提示词已提交，
Grok 没有桥接器，改用屏幕稳定与变化判断并在必要时补发 Enter。会话提示词
...
如果处理会话启动失败，诊断包仍会保留。报告按钮会实际调用所选 CLI 当前账号配置的模型，
并产生相应模型用量。
