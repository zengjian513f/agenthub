...
- **分层（顶栏树杈图标开关）**：打开后每条会话的子代理、以及由它发起的其他 CLI 会话缩进挂在它下面（可多层），跨目录的孩子离开自己的分组；整棵子树按最新活动排位，在跑的孩子排最前。每行行首是引导区：每一级祖先一根竖线，再一个 16px 的三角槽位（叶子留空），根行的三角与分组标题的三角同列，深一层的槽位正好在上一层槽位正下方——靠固定宽度对齐，不用绝对定位。子代理行的来源图标照常，右下角叠一个小分叉标记（与顶栏分层开关同一图形）；被发起的会话用自己的来源图标。发起者行的三角可收起整棵子树（收起状态存 localStorage）。发起者不在当前列表里（被筛掉、已删除）的会话仍作根显示。关掉开关即回到平铺，子代理仍只在详情标题下拉里切换。Claude 因上下文过长 continue 另起的 JSONL（文件尾 `continued-in`）只保留链上最新那一条出现在左栏；被取代的旧文件不展示。这和双 Esc 回退不同：双 Esc 仍在同一份 JSONL 里换枝。同一文件里的 `/compact` 也不另开会话。
