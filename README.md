...
- **管理操作**：Claude 子代理可在会话标题处下拉切换，各自保留独立时间线。下拉里主会话固定在前，子代理按结束时间倒序，每行带起止时间；还在跑的子代理没有结束时间、排最前并带绿点（服务端按子代理 transcript 的最后一条记录是否晚于父会话最近一次 task-notification / 前台 Agent tool_result 判定，父进程不在则一律不算）。运行中的会话显示停止按钮，停止后原位变为删除按钮。
