...
网页控制台默认由 tmux 托管会话；也可切换到不依赖 tmux 的自制会话宿主（Rust，见
[会话宿主](docs/session-host.md)），为 Windows 节点做准备。
...
Codex 回退生成子会话时，会弹窗询问是否隐藏父会话。选择保留或按 Esc 都会继续显示父会话；
选择隐藏只影响当前浏览器的列表，不删除记录，也不影响子会话继承历史。
选择会被记住，可在设置中的「回退父会话 → 恢复显示」找回已隐藏的父会话。
